...
{
    public class EmployeeJobDetailManager : RepositoryBaseManager<EmployeeJobDetail>, IEmployeeJobDetailService
    {
        private ApplicationDbContext _apiDbContext;

        public EmployeeJobDetailManager(ApplicationDbContext apiDbContext) : base(apiDbContext)
        {
        ...
        public EmployeeJobDetail GetDetailsById(int id)
        {
            return _apiDbContext.tblEmployeeJobDetails.Where(a => a.ID == id).FirstOrDefault();
        }


        public EmployeeJobDetail GetDetailsByEmployeeId(int EmployeeId)
        {
            return _apiDbContext.tblEmployeeJobDetails.Where(a => a.EmployeeID == EmployeeId).FirstOrDefault();
        }
        public EmployeeJobDetail GetLastJobEmployeeId(int EmployeeId)
        {
            return _apiDbContext.tblEmployeeJobDetails.Where(a => a.EmployeeID == EmployeeId).OrderBy(a => a.ID).LastOrDefault();
        }
